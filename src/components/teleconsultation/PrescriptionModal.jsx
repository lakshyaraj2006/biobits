...
export const PrescriptionModal = ({ caseData, isOpen, onClose }) => {
  ...
  const buildAudioSummary = () => {
    const medList = (caseData.prescription?.medicines || medicines)
      ...
      .join(' ');
    return `${t('rxTitle')}: ${caseData.patientName}. ${medList}.`;
  };
      ...
      <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg sm:text-xl font-extrabold text-white">
                  {isAlreadyReviewed ? t('rxTitle', 'Digital E-Prescription Slip') : t('writeRx', 'Write Digital Prescription')}
                </h3>
                <AudioVoiceButton
                  text={buildAudioSummary()}
                  label={t('audioRx', 'Audio Prescription')}
                  size="sm"
                  className="bg-white/20 text-white border-white/30"
                />
              </div>
              <p className="text-xs text-teal-200 mt-0.5">
                Case ID: {caseData.id} • {t('patientName', 'Patient')}: {caseData.patientName} ({caseData.age}y / {translateText(caseData.gender)})
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {isAlreadyReviewed && (
              ...
              >
                ...
                <span className="hidden sm:inline">{t('printSlip', 'Print Slip')}</span>
              </button>
            )}
            <button
              onClick={onClose}
        ...
        <div id="printable-prescription" className="p-5 sm:p-7 space-y-6 max-h-[75vh] overflow-y-auto">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xl font-black text-slate-900 tracking-tight">
                  {t('appTitle', 'Sahay')} <span className="text-emerald-600">SWASTHYA TELECLINIC</span>
                </span>
                <span className="text-[10px] font-bold bg-slate-100 text-slate-700 px-2 py-0.5 rounded border border-slate-300">
                  {t('govtLinked', 'Govt. PHC Network')}
                </span>
              </div>
              <p className="text-xs font-bold text-slate-700 mt-1">
                {t('storeAndForward', 'Primary Health Centre (PHC) Telemedicine Portal • Asynchronous Store-and-Forward')}
              </p>
              <p className="text-[11px] text-slate-500">
                {t('ayushmanAligned', 'Authorized under National Telemedicine Guidelines & Ayushman Bharat Digital Mission (ABDM)')}
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs">
            <div>
              <span className="text-slate-500 font-semibold block">{t('patientName', 'Patient Name')}</span>
              <strong className="text-slate-900 text-sm">{caseData.patientName}</strong>
            </div>
            <div>
              <span className="text-slate-500 font-semibold block">{t('age', 'Age')} / {t('gender', 'Gender')}</span>
              <strong className="text-slate-800">{caseData.age} {t('weeks', 'Years')} • {translateText(caseData.gender)}</strong>
            </div>
            <div>
              <span className="text-slate-500 font-semibold block">{t('villageLabel', 'Village / Hamlet')}</span>
              <strong className="text-slate-800">{caseData.village}</strong>
            </div>
            <div>
              <span className="text-slate-500 font-semibold block">{t('triageLevel', 'Triage Priority')}</span>
              <span className={`inline-block px-2 py-0.5 rounded-full font-bold text-[10px] uppercase ${
                ...
                {translateText(caseData.triageLevel)}
              </span>
            </div>
            ...
            <div className="col-span-2 sm:col-span-4 pt-2 border-t border-slate-200 flex flex-wrap gap-3 text-[11px] text-slate-700 font-medium">
              <span>🌡️ <strong>{t('temp', 'Temp')}:</strong> {caseData.vitals?.temperature}°F</span>
              <span>🩺 <strong>{t('bp', 'BP')}:</strong> {caseData.vitals?.bpSys}/{caseData.vitals?.bpDia} mmHg</span>
              <span>💓 <strong>{t('pulse', 'Pulse')}:</strong> {caseData.vitals?.pulse} {t('bpm', 'bpm')}</span>
              <span>🫁 <strong>{t('spO2', 'SpO2')}:</strong> {caseData.vitals?.spO2}%</span>
              <span>🩸 <strong>{t('sugar', 'Sugar')}:</strong> {caseData.vitals?.glucose} mg/dL</span>
            </div>
          </div>
          ...
          <div>
            <span className="text-xs font-bold text-slate-700 uppercase tracking-wider block mb-1.5">
              {t('symptomsTitle', 'Chief Complaints & Reported Symptoms')}:
            </span>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                  {t('medications', 'Prescribed Medicines & Dosage Schedule')}:
                </span>
              </div>

              {!isAlreadyReviewed && (
                <div className="no-print flex items-center gap-1.5">
                  <span className="text-xs text-slate-500 font-medium hidden sm:inline">{t('quickAdd', 'Quick Add')}:</span>
                  <select
                    onChange={(e) => {
                    ...
                    defaultValue=""
                  >
                    <option value="" disabled>+ {t('pickMedicine', 'Pick Standard Medicine')}</option>
                    {PRESET_MEDS.map((m) => (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs font-bold text-teal-800 mt-1">
                          ⏰ {t('timing', 'Timing')}: {med.timing}
                        </p>
                        <p className="text-xs text-slate-600 mt-0.5">
                          💡 {t('instructions', 'Instructions')}: {med.instruction}
                        </p>
                      </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          placeholder={t('medNamePlaceholder', 'Medicine name & strength (e.g. Paracetamol 650mg)')}
                        ...
                        <input
                          ...
                          placeholder={t('durationPlaceholder', 'Duration (e.g. 5 Days)')}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <input
                          ...
                          placeholder={t('timingPlaceholder', 'Timing (e.g. Morning & Night after food)')}
                        ...
                        <input
                          ...
                          placeholder={t('instPlaceholder', 'Special instructions (e.g. Take with warm water)')}
                          className="px-2.5 py-1.5 rounded-lg border border-slate-300 text-xs"
                        />
                      </div>
                    </div>
            ...
            {!isAlreadyReviewed && (
              ...
              >
                ...
                {t('addMedicine', 'Add Another Medicine')}
              </button>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-3.5 rounded-2xl bg-amber-50/70 border border-amber-200">
              <span className="text-xs font-bold text-amber-900 uppercase block mb-1">
                🥣 {t('dietCare', 'Diet & Home Care Advice')}:
              </span>
              {isAlreadyReviewed ? (
            ...
            <div className="p-3.5 rounded-2xl bg-rose-50/70 border border-rose-200">
              <span className="text-xs font-bold text-rose-900 uppercase block mb-1">
                ⚠️ {t('dangerSignsTitle', 'Danger Alarm Signs')}:
              </span>
              {isAlreadyReviewed ? (
          ...
          <div className="pt-4 border-t border-slate-200 flex flex-wrap items-end justify-between gap-4">
            <div>
              <span className="text-[10px] text-slate-500 font-bold uppercase">{t('authorizedDoctor', 'Authorized Medical Officer')}</span>
              ...
              <p className="text-[10px] text-emerald-700 font-bold mt-0.5">
                ✓ {t('digitalVerified', 'Digitally signed & verified for dispensing at local village Jan Aushadhi Kendra')}
              </p>
            </div>
            ...
            <div className="text-right">
              ...
              <span className="text-[10px] text-slate-500 font-bold">{t('digitalSignature', 'Digital Signature')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-50 p-4 border-t border-slate-200 flex items-center justify-between no-print">
          ...
          {!isAlreadyReviewed && (
            ...
            >
              ...
              {t('issueRx', 'Sign & Issue Prescription')}
            </button>
          )}
        </div>
      </div>
