...
export const PatientCaseCard = ({ caseItem, onOpenRx, onReview }) => {
  ...
  const audioSummary = `${t('patientName', 'Patient')}: ${caseItem.patientName}, ${t('symptomsTitle', 'Symptoms')}: ${caseItem.symptoms.map(s => translateText(s)).join(', ')}. ${isReviewed ? t('rxIssued', 'Prescription is ready.') : t('pendingReview', 'Pending doctor review.')}`;
      ...
      <div className="p-5 pb-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium mt-0.5 flex items-center gap-1.5">
                <span>{caseItem.age} {t('weeks', 'Yrs')} • {translateText(caseItem.gender)}</span>
                <span>•</span>
          ...
          <span className={`px-2.5 py-1 rounded-full text-[11px] font-extrabold uppercase tracking-wider ${badgeColor}`}>
            {translateText(caseItem.triageLevel)}
          </span>
        </div>
        ...
        <div className="mt-3.5 grid grid-cols-3 sm:grid-cols-4 gap-1.5 text-[11px]">
          <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 text-center">
            <span className="text-slate-400 block text-[10px]">{t('temp', 'Temp')}</span>
            <strong className={`font-bold ${caseItem.vitals.temperature >= 100.4 ? 'text-rose-600' : 'text-slate-800'}`}>
              {caseItem.vitals.temperature}°F
            </strong>
          </div>
          <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 text-center">
            <span className="text-slate-400 block text-[10px]">{t('bp', 'BP')}</span>
          ...
          <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 text-center">
            <span className="text-slate-400 block text-[10px]">{t('pulse', 'Pulse')}</span>
            <strong className="text-slate-800 font-bold">{caseItem.vitals.pulse} {t('bpm', 'bpm')}</strong>
          </div>
          <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 text-center hidden sm:block">
            <span className="text-slate-400 block text-[10px]">{t('spO2', 'SpO2')}</span>
