  ...
  const verifyEraktKosh = [
    {
      ...
      lastUpdated: 'Confirmed 10 min ago (e-RaktKosh Direct API)',
    ...
    },
    {
      ...
      status: 'Out of Stock',
      units: '0 Units Available',
      lastUpdated: 'Checked 1 hour ago',
      type: 'CHC Cabinet',
  ...
  const communityDonors = [
    {
      id: 'DONOR-882',
      group: 'O+',
      distance: '2.4 km away (Rampur Village)',
    ...
    },
    {
      id: 'DONOR-104',
      group: 'O+',
      distance: '5.8 km away (Balanagar)',
      availability: 'Available (Evening Only)',
      verified: true,
      ...
      <div className="bg-cream-bg rounded-3xl border border-cream-border w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="bg-brand-deep text-white p-5 flex items-center justify-between border-b border-rose-950">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-extrabold tracking-tight">Blood Coordination</h2>
              <p className="text-[11px] text-rose-200 font-medium">Sahay Integrated Blood Network</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          ...
          {stage === 'input' && (
            <form onSubmit={handleSearch} className="space-y-4 text-center">
              ...
              <div>
                <h3 className="text-sm font-extrabold text-text-dark">Search Verified Blood Bank Inventories</h3>
                <p className="text-xs text-text-muted max-w-md mx-auto">
                  Sahay queries the national e-RaktKosh platform and coordinates with verified local repositories first.
                </p>
              </div>

              <div className="max-w-xs mx-auto space-y-3">
                <div>
                  <label className="text-[10px] text-text-muted font-bold block mb-1">Required Blood Group</label>
                  <select
                    value={bloodGroup}
          ...
          {stage === 'searching' && (
            <div className="py-8 text-center space-y-4 animate-pulse">
              ...
              <h3 className="text-sm font-bold text-text-dark">Checking e-RaktKosh Registry...</h3>
              <p className="text-[11px] text-text-muted">Fetching live stocks from government banks & local PHC fridges.</p>
            </div>
          )}
          ...
          {stage === 'results' && (
            <div className="space-y-5 text-left">
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-text-dark">1. Official inventories (e-RaktKosh)</h3>
                  <p className="text-[10px] text-text-muted">Always request official inventory first to safeguard clinical quality.</p>
                </div>
                ...
                {verifyEraktKosh.map((b, idx) => (
                  <div key={idx} className="p-3 bg-white border border-cream-border rounded-2xl flex justify-between items-center gap-3">
                    <div>
                      <strong className="text-xs text-text-dark block">{b.bankName}</strong>
              ...
              <div className="space-y-3 pt-3 border-t border-cream-border">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <h3 className="text-xs font-bold uppercase tracking-wider text-text-dark">2. Community donor network fallback</h3>
                    <p className="text-[10px] text-text-muted">Use this community fallback pool if official stocks are unavailable.</p>
                  </div>
                </div>
                ...
                <div className="p-3 bg-stone-100 rounded-xl border border-cream-border flex gap-2">
                  ...
                  <p className="text-[9px] text-text-muted leading-normal">
                    <strong>Donor Privacy Protected:</strong> In compliance with data minimization guidelines, donors' names, phone numbers, and histories are hidden. Direct connection is brokered by the Care Desk operator ONLY after donor accepts the coordination request.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  {communityDonors.map((d) => (
                    <div key={d.id} className="p-3 bg-white border border-cream-border rounded-2xl flex items-center justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <strong className="text-xs text-stone-800">Verified {d.group} Donor ({d.id})</strong>
          ...
          {stage === 'requested' && (
            <div className="py-6 text-center space-y-4">
              ...
              <div>
                <h3 className="text-sm font-extrabold text-emerald-950">Match Requested Successfully</h3>
                <p className="text-xs text-emerald-800 max-w-sm mx-auto bg-emerald-50 p-2.5 rounded-xl border border-emerald-100 font-semibold mt-2">
                  Our Care Desk Operator is requesting authorization from {selectedDonor?.id}. Once accepted, transport alignment will begin automatically.
                </p>
              </div>
