...
export const OfflineSyncManager = () => {
  ...
  const audioSummary = `Offline database synchronization manager. BioBits stores all teleconsultation records, patient photos, and vaccination logs on your local device without needing internet. You currently have ${pendingSyncQueue.length} records pending synchronization to the PHC central server.`;
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white p-6 sm:p-8 rounded-3xl shadow-xl border border-indigo-800/40">
        <div className="max-w-3xl space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-200 text-xs font-bold border border-indigo-400/30">
            ...
            <span>Rural Offline-First Resilience • IndexedDB & Local Storage Cache</span>
          </div>

          <div className="flex items-center gap-3">
            <h1 className="text-2xl sm:text-4xl font-extrabold text-white">
              Offline Storage & Server Sync Manager
            </h1>
          ...
          <p className="text-xs sm:text-sm text-indigo-100/90 leading-relaxed font-medium">
            Engineered specifically for remote tribal hamlets and hilly rural pockets with zero/intermittent cellular network. All case entries, vitals, and vaccine marks remain 100% accessible and operable offline.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
          ...
          <p className="text-xs text-slate-600 leading-relaxed">
            Toggle offline simulation to test how ASHA field workers can record cases, take photos, and track vaccines in remote zero-network villages.
          </p>
          ...
          >
            ...
            <span>{isOffline ? 'Switch to Online Mode' : 'Simulate Offline Village Mode'}</span>
          </button>
        </div>
        ...
        <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
          ...
          <p className="text-xs text-slate-600 leading-relaxed">
            {pendingSyncQueue.length === 0
              ? 'All local records are synchronized with the PHC District Central Server.'
              : `${pendingSyncQueue.length} offline case file(s) and vaccine updates are waiting in device storage for cloud sync.`}
          </p>
          ...
          >
            ...
            <span>{isSyncing ? 'Synchronizing Data...' : 'Sync to Central Server Now'}</span>
          </button>
        </div>
        ...
        <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-sm space-y-3">
          ...
          <div className="space-y-1.5 text-xs text-slate-700 font-medium">
            <div className="flex justify-between py-1 border-b border-slate-100">
              <span>Patient Case Files:</span>
              <strong className="text-slate-900 font-bold">Stored Locally</strong>
            </div>
            <div className="flex justify-between py-1 border-b border-slate-100">
              <span>Epidemic Anomaly Logs:</span>
              <strong className="text-slate-900 font-bold">Persistent Cache</strong>
            </div>
            <div className="flex justify-between py-1">
              <span>UIP Immunization Cards:</span>
              <strong className="text-slate-900 font-bold">Zero-Loss Local DB</strong>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div>
            <h3 className="text-base sm:text-lg font-extrabold text-slate-900">
              Local Offline Transactions Log ({pendingSyncQueue.length})
            </h3>
            <p className="text-xs text-slate-500">
              Transactions queued while offline; automatically uploaded once internet connectivity resumes.
            </p>
          </div>
          ...
          {pendingSyncQueue.length > 0 && (
            ...
            >
              Sync All ({pendingSyncQueue.length})
            </button>
          )}
        </div>
        ...
        {pendingSyncQueue.length === 0 ? (
          <div className="py-12 text-center space-y-2">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              No offline transactions are waiting. You can create new case files or mark vaccines offline, and they will appear here.
            </p>
            <div className="pt-2">
              <Link
                to="/teleconsult"
              ...
              >
                Log a test case in Teleconsultation →
              </Link>
            </div>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-700 font-bold border-b border-slate-200">
                  ...
                  <th className="py-2.5 px-3">Local Timestamp</th>
                ...
                {pendingSyncQueue.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    ...
                    <td className="py-3 px-3 text-slate-700">
                      {item.patientName ? `Patient: ${item.patientName} (${item.village})` : JSON.stringify(item)}
                    </td>
                    <td className="py-3 px-3 text-slate-500 text-[11px]">
                      {new Date(item.timestamp).toLocaleTimeString()}
                    </td>
                    <td className="py-3 px-3 text-right">
                      <span className="text-amber-700 font-bold text-xs">Queued locally</span>
                    </td>
                  </tr>
                ))}
              </tbody>
